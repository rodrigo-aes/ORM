...
export default class SelectQueryHandler<T extends EntityTarget> {
    protected metadata: EntityMetadata

    private _properties: SelectPropertyType<T>[] = []
    private _count: CountQueryHandler<T>[] = []
    ...
    public count(
        ...
        const handler = new CountQueryHandler(this.target, this.alias)
    ...
    ): SelectPropertyKey<InstanceType<T>> | CaseQueryHandler<T> {
        switch (typeof property) {
            case "string": return property
            case "function":
                const caseClause = new CaseQueryHandler(
                    this.target,
                    this.alias
                )
